...
const commands = {
  ...
  color: (args) => {
    function parseColor(input) {
      if (input.startsWith("rgb")) {
        // Если введено "rgb", считаем остаток строки как аргументы
        const rgbValues = input.substring(3).trim();
        return `rgb(${rgbValues})`;
      } else {
        // Введено что-то иное, самостоятельно определяем цвет
    ...
    if (args.length === 3) {
      ...
      if (operation === "encode" && encryptionType === "base64") {
        ...
        try {
          ...
          // Convert the decoded text to a byte array
  ...
  del: (args) => {
    if (args.length > 0) {
      ...
      if (isValidPathFormat(pathToDelete)) {
        ...
        setCookie("deletedPaths", JSON.stringify(deletedPaths)); // Обновляем куки
  ...
  note: (args) => {
    if (args.length >= 1) {
      ...
      // Получаем текущие заметки из Local Storage
      let existingNotes = localStorage.getItem("notes");
      if (!existingNotes) {
        existingNotes = "";
      }

      // Добавляем новую заметку и обновляем Local Storage
      const updatedNotes = `${existingNotes}\n- ${noteText}`;
      localStorage.setItem("notes", updatedNotes);

      addToConsole("Note added and saved.", false);
    } else {
      // Выводим ранее сохраненные заметки из Local Storage
      let existingNotes = localStorage.getItem("notes");
      if (!existingNotes) {
  ...
  regedit: async () => {
    try {
      const response = await fetch('http://localhost:3000/regedit', { method: 'POST' });

      if (response.ok) {
        addToConsole('Registry Editor (regedit) opened successfully on the server!', false);
      } else {
        addToConsole('Error opening Registry Editor on the server', false);
      }
    } catch (error) {
